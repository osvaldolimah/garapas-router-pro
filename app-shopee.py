...
TERMOS_ANULADORES = ['FRENTE', 'LADO', 'PROXIMO', 'VIZINHO', 'DEFRONTE', 'ATRAS', 'DEPOIS', 'PERTO', 'VIZINHA', 'AO LADO', 'EM FRENTE', 'OPPOSTO', 'PRÓXIMO A', 'PERTO DE', 'AO LADO DE', 'DE FRENTE PARA', 'DE FRENTE A',
                     'DE FRENTE', 'EM FRENTE A', 'EM FRENTE PARA', 'OPPOSTO A', 'OPPOSTO DE', 'AO LADO DE', 'PRÓXIMO DE', 'PERTO DE', 'VIZINHO A', 'VIZINHO DE', 'DEPOIS DE', 'DEPOIS A', 'ATRÁS DE', 'ATRÁS A',
                       'PERTO DE', 'PRÓXIMO A', 'PRÓXIMO DE', 'PROX. A', 'PROX. DE', 'AO LADO DE', 'AO LADO A', 'DE FRENTE PARA', 'DE FRENTE A', 'EM FRENTE A', 'EM FRENTE PARA', 'OPPOSTO A', 'OPPOSTO DE', 'PROX',
                       'PROX.']
...
st.markdown("""
    ...
    .sos-btn {
        ...
        text-align: center; text-decoration: none; font-weight: 800; color: #333;
        box-shadow: 0 2px 5px rgba(0,0,0,0.05); transition: 0.2s;
        font-size: 1.1rem;
    }
    .sos-btn:hover { background-color: #f8f8f8; border-color: #bbb; transform: translateY(-1px); }
    </style>
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for element in data.get('elements', []):
                ...
                if 'borracharia' in nome_lower or shop == 'tyres':
                    tipo_fmt = "🔘 Borracharia"; icone = "🔘"
                elif 'reboque' in nome_lower:
                    tipo_fmt = "🛻 Reboque"; icone = "🛻"
                else:
                    tipo_fmt = "🔧 Oficina"; icone = "🔧"
                e_lat, e_lon = None, None
                if 'lat' in element and 'lon' in element:
                    e_lat = element.get('lat'); e_lon = element.get('lon')
                elif 'center' in element:
    ...
    if st.session_state.df_cache is not None and st.session_state.get('up_padrao_bytes') is not None:
        ...
        if st.button("📊 PROCESSAR MÚLTIPLAS GAIOLAS", key="btn_m_tab2", use_container_width=True):
            ...
            if not lista:
                ...
                st.warning("⚠️ Limite excedido: Digite no máximo 6 gaiolas por vez.")
            else:
