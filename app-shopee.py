...
# --- SISTEMA DE DESIGN (CSS RESPONSIVO - MARCO ZERO) ---
st.markdown("""
    <style>
...
# --- FUNÇÕES DO AGENTE IA (ADICIONAL) ---
def inicializar_ia():
...
def agente_ia_treinado(client, df, pergunta):
    prompt = f"Você é o Waze Humano em Fortaleza. Use estas regras: {TERMOS_COMERCIAIS}. Dados: {df.head(150).to_string()}. Pergunta: {pergunta}"
    try:
        response = client.models.generate_content(model='gemini-2.5-flash', contents=prompt)
        return response.text
    except Exception as e: return f"IA: {e}"
...
if arquivo_upload:
    ...
    with tab1: # CONTEÚDO MARCO ZERO
        gaiola_unica = st.text_input("Gaiola única", placeholder="Ex: C-42", key="gui").strip().upper()
    ...
    with tab2: # CONTEÚDO MARCO ZERO
        codigos_m = st.text_area("Gaiolas (uma por linha)", placeholder="B-50\nC-42", key="c_m")
    ...
    # --- RESULTADOS MÚLTIPLAS GAIOLAS (A PARTE QUE EU ESQUECI) ---
    if st.session_state.modo_atual == 'multiplas' and st.session_state.resultado_multiplas:
        ...
        g_enc_lista = [codigo for codigo, dados in res.items() if dados['encontrado']]
        
        if g_enc_lista:
            st.markdown("##### ✅ Selecione as gaiolas:")
            cols = st.columns(3)
            selecionadas = []
            for idx, codigo in enumerate(g_enc_lista):
                with cols[idx % 3]:
                    if st.checkbox(f"**{codigo}** ({res[codigo]['pacotes']} pacotes)", key=f"chk_{codigo}"):
                        selecionadas.append(codigo)
            
            if selecionadas and st.button(f"📥 GERAR {len(selecionadas)} PLANILHA(S) SELECIONADA(S)", use_container_width=True):
                xl_m = pd.ExcelFile(arquivo_upload)
                for gaiola in selecionadas:
                    target_l = limpar_string(gaiola)
                    for aba in xl_m.sheet_names:
                        ...
                        if idx_g is not None:
                            res_ind = processar_gaiola_unica(df_r, gaiola, idx_g)
                            if res_ind:
                                buf_ind = io.BytesIO()
                                with pd.ExcelWriter(buf_ind, engine='openpyxl') as w: res_ind['dataframe'].to_excel(w, index=False)
                                st.download_button(label=f"📄 Baixar Rota {gaiola}", data=buf_ind.getvalue(), file_name=f"Rota_{gaiola}.xlsx", key=f"dl_{gaiola}", use_container_width=True)
                                break
else:
